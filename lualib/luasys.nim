...
type
  ...
  LuaRef = ref RootObj # Fake type.

var
  framePtr {.importc, nodecl, volatile.}: PCallFrame
  excHandler {.importc, nodecl, volatile.}: int = 0
  lastLuaError {.importc, nodecl, volatile.}: PLuaError = nil

{.push stacktrace: off, profiler:off.}

proc isNimException(): bool {.asmNoStackFrame.} =
  asm "return `lastLuaError` && `lastLuaError`.m_type;"

proc getCurrentException*(): ref Exception {.compilerRtl, benign.} =
  if isNimException(): result = cast[ref Exception](lastLuaError)

proc getCurrentExceptionMsg*(): string =
  if lastLuaError != nil:
    if isNimException():
      return cast[Exception](lastLuaError).msg
    else:
      var msg: cstring
      {.emit: """
      if `lastLuaError`.message ~= nil then
        `msg` = `lastLuaError`.message
      end
      """.}
      if not msg.isNil:
        return $msg
...
proc reraiseException() {.compilerproc, asmNoStackFrame.} =
  if lastLuaError == nil:
    ...
    if excHandler == 0:
      if isNimException():
        unhandledException(cast[ref Exception](lastLuaError))

    asm "throw lastLuaError;"
...
proc cstrToNimstr(c: cstring): string {.asmNoStackFrame, compilerproc.} =
  ...
  return result;
  """.}
proc toLuaStr(s: string): cstring {.compilerproc.} =
  proc fromCharCode(c: char): cstring {.importc: "string.char".}
  proc join(x: openArray[cstring]; d = cstring""): cstring {.
    importc: "table.concat".}
  proc decodeURIComponent(x: cstring): cstring {.
    importc: "decodeURIComponent".}
  proc toHexString(c: char; d = 16): cstring {.importcpp: "string.format('%02X', string.byte(#))".}
  proc log(x: cstring) {.importc: "print".}
...
proc nimCopy(dest, src: LuaRef, ti: PNimType): LuaRef {.compilerproc.}
proc nimCopyAux(dest, src: LuaRef, n: ptr TNimNode) {.compilerproc.} =
  case n.kind
  ...
  of nkCase:
    ...
    """
proc nimCopy(dest, src: LuaRef, ti: PNimType): LuaRef =
  case ti.kind
  ...
  of tyString:
    ...
    result = src
proc genericReset(x: LuaRef, ti: PNimType): LuaRef {.compilerproc.} =
  asm "`result` = null;"
  case ti.kind
  of tyPtr, tyRef, tyVar, tyNil:
    if isFatPointer(ti):
  ...
  of tyArrayConstr, tyArray:
    ...
    discard
proc arrayConstr(len: int, value: LuaRef, typ: PNimType): LuaRef {.
